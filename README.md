...
- 仓库：<https://github.com/kevlns/u-cli>
- 包名：`@kevlns/u-cli`（当前 `private: true`，尚未发布）
...
> **缓存目录说明**：用户缓存默认为 `%LOCALAPPDATA%\editor-pipeline-cli`；该目录名沿用历史名称，以避免破坏已通过验证的缓存/收据契约。工程内的安装事务目录 `Library/editor-pipeline-cli` 同理保留。
...
```bash
# 只读检查工程与路由
u-cli doctor C:\Projects\Debussy\debussy-client
...
# 预览 Pipeline 安装（不写入）
u-cli pipeline install C:\Projects\Debussy\debussy-client --dry-run

# 正式安装（先关闭目标工程的 Unity Editor）
u-cli pipeline install C:\Projects\Debussy\debussy-client

# CLI + Pipeline 一键准备
u-cli setup C:\Projects\Debussy\debussy-client

# 执行 Unity Pipeline 命令（自动绑定目标工程）
u-cli exec C:\Projects\Debussy\debussy-client -- command editor_status
u-cli exec C:\Projects\Debussy\debussy-client -- command get_scene_hierarchy
...
发布后也可 `npx @kevlns/u-cli ...` 使用；当前版本尚未发布（`private: true`）。
...
- 包名为 `@kevlns/u-cli`、tgz 文件名为 `kevlns-u-cli-*`；
- 不出现：
...
带临时 `--prefix` 的全局安装、`npx --package <本地tgz>`；每种都执行 `u-cli --version` 与
`u-cli routes`。**不会**向任何 registry 发布。报告：`C:/tmp/u-cli-package-test-report.json`。
...
- `e2e:windows` 使用**隔离缓存**（干净环境）或显式复用已验证缓存，创建 `C:/tmp/u-cli-e2e/` 下独立临时工程，全部结束后杀掉它启动的所有 Unity 进程并清理临时工程；报告写入 `C:/tmp/u-cli-e2e-report.json`。
...
- `.github/workflows/e2e-windows.yml`：**self-hosted** runner（标签 `self-hosted/windows/unity-2022.3.62f3c1`）上跑 Unity E2E；仅 `workflow_dispatch` 与受保护 `v*` tag 触发，不跑 pull_request，不发布；权限最小化（contents: read）；产物 `u-cli-e2e-report`。
